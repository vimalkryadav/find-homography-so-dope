...
# Two routes agreeing this closely (in metric units) are treated as confirming
# each other. Well above the residual of a good fit, far below a real error.
CONSENSUS_TOL = 0.004

# One very strong edge should not outvote several independent agreeing routes.
INLIER_VOTE_CAP = 400

# How far the alternatives must outweigh a strong direct fit to overrule it.
OVERRIDE_FACTOR = 3.0
...
class _Scene:
    ...
    def _agreement(self, Ha, Hb, target):
        """Distance between two candidates, in the competition's own metric."""
        w1, h1 = self.size(1)
    ...
    def _consensus(self, target, cands):
        """Pick the route the other routes most agree with.

        Inlier count alone cannot tell a confident-but-wrong direct fit from a
        correct one; independent routes through different intermediates can,
        because they only agree with each other when they are all right.
        """
        usable = [c for c in cands
                  if c[0] is not None and not _degenerate(c[0], self.size(1))]
        if not usable:
            return None
        if len(usable) == 1:
            return usable[0][0]
        ...
        for c in usable:
            ...
            for o in usable:
                if o is c:
                    continue
                if self._agreement(c[0], o[0], target) < CONSENSUS_TOL:
                    support += min(o[1], INLIER_VOTE_CAP)
            scored.append((support, c))

        best_support = max(s for s, _ in scored)
        if best_support <= 0.0:
            # nothing agrees with anything: fall back to the old preference order
            return max(usable, key=lambda c: (_clean(c[0], self.size(1)),
                                              c[1], -len(c[2])))[0]
        top = [c for s, c in scored if s == best_support]
        return max(top, key=lambda c: (_clean(c[0], self.size(1)),
                                       c[1], -len(c[2])))[0]

    def _solve(self):
        targets = [k for k in self.idx if k != 1]

        # direct edges first: the identity test needs nothing more
        ...
        for k in targets:
            e = direct.get(k)
            strong = (e is not None and e[1] >= MIN_DIRECT_INLIERS
                      and not _degenerate(e[0], self.size(1)))
            cands = self._candidates(k)

            if strong:
                # Chained routes share edges, so they agree with each other
                # whether or not they are right. Their agreement is therefore
                # only trusted to overrule a strong direct fit when the margin
                # is wide; otherwise the direct fit stands.
                best = self._veto(k, e[0], cands)
            else:
                best = self._consensus(k, cands)
    ...
    def _veto(self, target, direct_H, cands):
        """Keep the direct fit unless the alternatives overwhelmingly disagree."""
        usable = [c for c in cands
                  if c[0] is not None and not _degenerate(c[0], self.size(1))]
        if len(usable) < 2:
            return direct_H
        ...
        def support(H):
            return sum(min(o[1], INLIER_VOTE_CAP) for o in usable
                       if self._agreement(H, o[0], target) < CONSENSUS_TOL
                       and o[0] is not H)

        direct_support = support(direct_H)
        alts = [c for c in usable if c[0] is not direct_H]
        if not alts:
            return direct_H
        best_alt = max(alts, key=lambda c: (support(c[0]), c[1]))
        alt_support = support(best_alt[0])

        if alt_support > max(direct_support * OVERRIDE_FACTOR, INLIER_VOTE_CAP):
            return best_alt[0]
        return direct_H
