...
def _plausible(H, wh):
    """Reject a homography that folds, mirrors or collapses the image.

    A correct H maps the image rectangle to a convex quadrilateral of similar
    scale with every corner in front of the camera. Degenerate RANSAC consensus
    sets fail all three tests at once, and composing them amplifies the damage,
    so they are dropped rather than chained through.
    """
    if H is None:
        return False
    w, h = wh
    ...
    if not np.all(np.isfinite(p)) or np.any(np.abs(p[:, 2]) < 1e-9):
        return False
    if not np.all(np.sign(p[:, 2]) == np.sign(p[0, 2])):
        return False          # a corner has crossed the horizon
    q = p[:, :2] / p[:, 2:3]
...
class _Scene:
    ...
    def _edge(self, i, j):
        ...
        if H is not None and not _plausible(H, self.size(i)):
            H = None
        if H is not None:
            self.edges[(i, j)] = (H, n)
    ...
    def _best_path(self, target):
        """Path 1 -> target maximising the smallest inlier count along it."""
        others = [n for n in self.idx if n not in (1, target)]
        best_path, best_bottleneck = None, -1
        for r in range(0, min(3, len(others)) + 1):
            for mid in itertools.permutations(others, r):
                ...
                if any(e not in self.edges for e in edges):
                    continue
                bottleneck = min(self.edges[e][1] for e in edges)
                if bottleneck > best_bottleneck:
                    best_path, best_bottleneck = path, bottleneck
        if best_path is None:
            return None
        H = IDENTITY.copy()
        for t in range(len(best_path) - 1):
            H = self.edges[(best_path[t], best_path[t + 1])][0] @ H
        return _sanitise(H)
    ...
    def _solve(self):
        ...
        for k in targets:
            e = direct.get(k)
            H = e[0] if e is not None and e[1] >= MIN_DIRECT_INLIERS else None
            if H is None:
                chained = self._best_path(k)
                H = chained if chained is not None else (e[0] if e else None)
            if H is not None and not _plausible(H, self.size(1)):
                H = None
            if H is None:
                self.unresolved.add(k)
            else:
                H = self._guided_refine(1, k, H)
                if not _plausible(H, self.size(1)):
                    H = None
            self.answers[k] = H if H is not None else IDENTITY.copy()
