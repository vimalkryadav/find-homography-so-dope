...
# How far the inlier residual may grow before a refinement is read as a slide.
# Every refinement that proved correct against the training ground truth stayed
# at or under 1.62; the ones that proved wrong sat at 2.05 and above. The floor
# keeps the ratio from being meaningless when the residual is already sub-pixel.
ECC_RESID_TOL = 1.7
ECC_RESID_FLOOR = 1.0

# A dense matcher earns its place only in a narrow band. Above this many SIFT
# inliers the sparse fit is already more precise; at or below the lower bound the
# pair is hard enough that the dense matcher fails too (scene_033 returns 8
# matches and a useless homography).
DENSE_MIN_INLIERS = 30
DENSE_MAX_INLIERS = 200

# Whether a refinement is an improvement is decided photometrically, by warping
# one image onto the other and measuring the correlation. Judging it by how far
# the reference corners moved rejects the pairs that need it most: near the
# horizon a tiny, correct change to H moves a corner enormously.
...
class _Scene:
    ...
    def _inlier_residual(self, i, j, H):
        """Median reprojection error of the matched inliers, in pixels.

        Measured where the features actually are, which is what separates a
        genuine correction from a slide: ECC moving a near-horizon corner a long
        way can still be right, but it cannot be right while abandoning the
        correspondences the homography was built from.
        """
        pts = self.corr.get((i, j))
        if pts is None or H is None:
            return None
        src, dst = pts
        if len(src) < MIN_MATCHES:
            return None
        with np.errstate(over="ignore", invalid="ignore", divide="ignore"):
            p = np.column_stack([src, np.ones(len(src))]) @ np.asarray(H, dtype=np.float64).T
            w = p[:, 2]
            ok = np.isfinite(w) & (np.abs(w) > 1e-12)
            if ok.sum() < MIN_MATCHES:
                return np.inf
            q = p[ok, :2] / w[ok, None]
            d = np.linalg.norm(q - dst[ok], axis=1)
        if not np.all(np.isfinite(d)):
            return np.inf
        return float(np.median(d))

    def _dense_candidate(self, i, j):
        """A dense-matcher homography, for the band where sparse matching thins out.

        Returned unrefined on purpose: photometric polishing pulls these back to
        the sparse answer, which for exactly these pairs is the worse one.
        """
        e = self.edges.get((i, j))
        n = e[1] if e else 0
        if not (DENSE_MIN_INLIERS <= n <= DENSE_MAX_INLIERS):
            return None
        try:
            from dense_matcher import dense_homography
            H = dense_homography(os.path.join(self.folder, f"{i}.png"),
                                 os.path.join(self.folder, f"{j}.png"))
        except Exception:
            return None
        H = _sanitise(H)
        if H is None or _degenerate(H, self.size(i)):
            return None
        return H

    def _photo_score(self, i, j, H):
        """Correlation between image i and image j warped into its frame."""
        a = self._grayscale(i)
        b = self._grayscale(j)
        ...
        if not np.all(np.isfinite(inv)):
            return -np.inf
        h, w = a.shape[:2]
        warped = cv2.warpPerspective(b, inv, (w, h), flags=cv2.INTER_LINEAR,
                                     borderMode=cv2.BORDER_CONSTANT, borderValue=0)
        mask = (cv2.warpPerspective(np.ones_like(b), inv, (w, h),
                                    flags=cv2.INTER_NEAREST) > 0.5).astype(np.uint8)
        if int(mask.sum()) < 0.05 * h * w:
            return -np.inf
        try:
            return float(cv2.computeECC(a, warped, mask))
        except cv2.error:
            return -np.inf
    ...
    def _solve(self):
        ...
        for k in targets:
            ...
            dense = self._dense_candidate(1, k)
            if dense is not None:
                self.answers[k] = dense
                continue

            polished = self._ecc_refine(1, k, best)
            if polished is not None and not _degenerate(polished, self.size(1)):
                if self._photo_score(1, k, polished) > self._photo_score(1, k, best):
                    was = self._inlier_residual(1, k, best)
                    now = self._inlier_residual(1, k, polished)
                    if (now is None or was is None
                            or now <= max(was * ECC_RESID_TOL, ECC_RESID_FLOOR)):
                        best = polished
            self.answers[k] = best
