...
class _Scene:
    ...
    def _polish(self, i, j, H):
        """Photometrically refine a single edge before anything composes it.

        A route like 1 -> 5 -> 6 can end up near the horizon even though neither
        link is, and ECC slides when it runs on that composed transform. Each
        link on its own is a mild transform where the alignment is well behaved,
        so refining the edges and composing afterwards keeps the precision
        without ever asking ECC to solve the ill-conditioned problem.
        """
        cand = self._ecc_refine(i, j, H)
        if cand is None or _degenerate(cand, self.size(i)):
            return H
        if self._photo_score(i, j, cand) <= self._photo_score(i, j, H):
            return H
        was = self._inlier_residual(i, j, H)
        now = self._inlier_residual(i, j, cand)
        if was is None or now is None:
            return H
        if now > max(was * ECC_RESID_TOL, ECC_RESID_FLOOR):
            return H
        return cand
