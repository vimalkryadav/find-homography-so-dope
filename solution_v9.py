...
# Bundling is meant to polish an answer, not move it somewhere new. A move this
# small is safe whatever the evidence; a larger one is only accepted where the
# direct match was too weak to be worth defending. Tuning the size of an
# acceptable move against the training scenes just fits the training scenes, so
# the decision is made on the strength of the evidence instead.
BUNDLE_FREE_DRIFT = 0.01

# Past that, a solve that relocates everything has diverged rather than converged.
...
class _Scene:
    ...
    def _bundle(self):
        ...
        for k in order:
            drift = self._metric_gap(self.answers[k], tuned[k], k)
            if drift <= BUNDLE_FREE_DRIFT:
                self.answers[k] = tuned[k]
                continue
            direct = self.edges.get((1, k))
            if direct is None or direct[1] < MIN_DIRECT_INLIERS:
                self.answers[k] = tuned[k]
